...
class SellConfirmPayoutTest(PipelineBaseTest):
    def test_sell_trade_to_submitted_address(self):
        ...
        trade.submit_payout_address(trade.taker_index)
    ...
    def test_sell_trade_to_submitted_invoice(self):
        trade = Trade(self.client, maker_form=maker_form_sell_with_range)
        trade.publish_order()
        trade.take_order()
        trade.take_order_third()
    ...
    def test_sell_trade_to_confirm_fiat_sent_LN(self):
        trade = Trade(self.client, maker_form=maker_form_sell_with_range)
        trade.publish_order()
        trade.take_order()
        trade.take_order_third()
        ...
        trade.undo_confirm_sent(trade.taker_index)
        data = trade.response.json()
        self.assertEqual(trade.response.status_code, 200)
        self.assertEqual(data["status_message"], Order.Status(Order.Status.CHA).label)
    ...
    def test_sell_trade_to_confirm_fiat_received_LN(self):
        trade = Trade(self.client, maker_form=maker_form_sell_with_range)
        trade.publish_order()
        trade.take_order()
        trade.take_order_third()
    ...
    def test_sell_successful_LN(self):
        trade = Trade(self.client, maker_form=maker_form_sell_with_range)
        trade.publish_order()
        trade.take_order()
        trade.take_order_third()
        ...
        self.assertTrue(data["is_fiat_sent"])
        self.assertFalse(data["is_disputed"])
        self.assertIsHash(data["taker_summary"]["preimage"])
        self.assertIsHash(data["taker_summary"]["payment_hash"])
        self.assert_order_logs(data["id"])

    def test_sell_successful_onchain(self):
        trade = Trade(self.client, maker_form=maker_form_sell_with_range)
        trade.publish_order()
        trade.take_order()
        trade.take_order_third()
        ...
        self.assertTrue(data["is_fiat_sent"])
        self.assertFalse(data["is_disputed"])
        self.assertIsInstance(data["taker_summary"]["address"], str)
        self.assertIsHash(data["taker_summary"]["txid"])
        self.assert_order_logs(data["id"])

    def test_sell_review_order(self):
        trade = Trade(self.client, maker_form=maker_form_sell_with_range)
        trade.publish_order()
    ...
    def test_sell_lightning_payment_failed(self):
        trade = Trade(self.client, maker_form=maker_form_sell_with_range)
        trade.publish_order()
        trade.take_order()
        trade.take_order_third()
        ...
        self.assertEqual(
            notifications_data[0]["title"],
            f"⚡❌ Hey {taker_nick}, the lightning payment on your order with ID {str(trade.order_id)} failed.",
        )
